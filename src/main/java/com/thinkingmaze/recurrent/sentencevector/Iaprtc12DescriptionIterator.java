 ...
 * @author Alex Black
 */
public class Iaprtc12DescriptionIterator implements DataSetIterator {
	...
	public Iaprtc12DescriptionIterator(String path, int miniBatchSize, int exampleSize, int numExamplesToFetch, int examplesPerEpoch ) throws IOException {
	 ...
	 */
	public Iaprtc12DescriptionIterator(String textFilePath, Charset textFileEncoding, int miniBatchSize, int exampleLength, int numExamplesToFetch,
	...
	public DataSet next(int num) {
		...
		INDArray input = Nd4j.zeros(num, exampleLength+numCharacters, numExamplesToFetch);
		INDArray labels = Nd4j.zeros(num, numCharacters, numExamplesToFetch);
		
		for (int i = 0; i < num; i++){
			int idx = (int) (rng.nextDouble()* currList.size()) ;
			int preChar = idx + numCharacters;
